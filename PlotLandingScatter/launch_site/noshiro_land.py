...
class NoshiroLand(LaunchSite):
    ...
    def __init__(self, elevation, magnetic_dec_exist, result_dir, glp_list, launch_LLH, safety_area, safety_exist):
        ...
        # 点火点
        # fire_ENU = cd.LLH2ENU(launch_LLH, np.array([34.735390,139.421377, 0.0]))
        self.wait_ENU = cd.LLH2ENU(self.launch_LLH, np.array([40.138237, 139.986642, 0.0]))
    ...
    def judge_inside(self):
        ...
        jic_wait = JudgeInsideCircle(self.wait_ENU, 50.0)

        judge_list = []
        for point_tra, point_par in zip(self.glp_tra.pos_ENU_array, self.glp_par.pos_ENU_array):
            judge_tra_list = [jip.judge_inside(point) for point in point_tra]
            judge_par_list = [jip.judge_inside(point) for point in point_par]
    ...
    def output_kml(self):
        self.trajectory.output_kml(self.launch_LLH, self.magnetic_dec, simplekml.Color.teal, 0, 0, 0, 0, self.safety_LLH, self.safety_exist)
        self.parachute.output_kml(self.launch_LLH, self.magnetic_dec, simplekml.Color.indianred, 0, 0, 0, 0, self.safety_LLH, self.safety_exist)